...
// ⭐ Routes pour la vérification d'email - Utiliser request()
/*
...
Route::get('email/verify/{id}/{hash}', function () {
    // Géré automatiquement par Laravel via VerifyEmailController
})->middleware(['auth', 'signed', 'throttle:6,1'])->name('verification.verify');

// ⭐ CORRECTION : Utiliser request() au lieu de Request $request
...
Route::get('/', function () {
    ...
    $services = \App\Models\Service::with('images')
        ...
        ->map(function ($service) {
            return [
                ...
                'images'      => $service->images->map(fn($img) => [
                    'url' => $img->url,
                    'nom' => $img->nom,
                ])->toArray(),
            ];
    ...
    return Inertia::render('Welcome', [
            ...
            ->map(function ($res) {
                ...
                return [
                    ...
                    'concour' => $res->concour ? [
                        ...
                        'service' => $res->concour->service ? ['nom' => $res->concour->service->nom] : null
                    ] : null,
        ...
        'servicesWithImages' => $services, // ⭐
    ]);
})->name('welcome');

Route::get('/api/service/{service}/concours', function ($serviceId) {
    $concours = \App\Models\Concour::where('service_id', $serviceId)
        ->where('statut', 'Actif')
        ->select('id', 'intitule', 'diplome_min', 'date_limite', 'age', 'service_id')
        ->get();

    return response()->json(['concours' => $concours]);
})->name('api.service.concours');
// =============================================
// 2. ROUTES POUR TOUS LES UTILISATEURS AUTHENTIFIÉS
// =============================================

Route::middleware(['auth'])->group(function () {
    Route::post('/theme/color', function (Request $request) {
        $request->validate([
            'color' => 'required|string|in:emerald,blue,purple,orange,pink,red,cyan,amber'
        ]);
        ...
        return response()->json(['color' => $user->theme_color]);
    })->name('theme.color');
    ...
    Route::get('/candidat-postuler', [CandidatPostulerController::class, 'index'])
        ->name('candidat-postuler.index');
    ...
    // ⭐ Route pour annuler une candidature
    Route::delete('/candidature/{id}/cancel', [CandidatDossierController::class, 'cancel'])->name('candidature.cancel');
    });
...
Route::middleware(['auth', 'adminMiddleware'])->group(function () {
    ...
    Route::middleware(['auth'])->group(function () {
        Route::get('/tracabilite', [TracabiliteController::class, 'index'])
            ->name('tracabilite.index')
            ->middleware('permission:voir dashboard');
    });
    ...
    Route::prefix('concours-historique')->name('concours-historique.')->group(function () {
        ...
        // ⭐ NOUVELLE ROUTE
        Route::get('/export-nina', [ConcourHistoriqueController::class, 'exportNina'])->name('export-nina');
    });
...
Route::middleware(['auth', 'superAdminMiddleware', 'superadmin.general'])->group(function () {
    // --- UTILISATEURS ---
