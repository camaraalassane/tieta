...
<body>
    ...
    <div style="margin-top: 5px;">
        @php
            // ⭐ Ajout de TS, TSS, DES, et Maitrise avec accent pour l'affichage
            $diplomes = [
                'DEF' => 'DEF',
                'CAP' => 'CAP',
                'BT' => 'BT',
            ...
            $count = 0;
        @endphp
        @foreach($diplomes as $colonne => $nomAffiche)
            @if(!empty($profil->$colonne))
                <span class="diplome-tag">{{ $nomAffiche }}</span>
                @php $count++; @endphp
            @endif
        @endforeach
        @if($count == 0)
