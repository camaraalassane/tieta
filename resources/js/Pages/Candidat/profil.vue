...
// ⭐ CORRECTION : utiliser dip.toLowerCase() pour que les champs du form
// correspondent aux noms attendus par le contrôleur (def, bac, maitrise, etc.)
const form = useForm({
    nom: props.user?.name || "",
    ...
    carte_identite: null,
    permis: null,
    ...Object.fromEntries(diplomesList.map((d) => [d.toLowerCase(), null])),
});
            ...
            <form @submit.prevent="submit" class="sakai-card p-6 md:p-10">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    <div class="flex flex-col gap-6">
                        ...
                        <!-- DIPLÔMES -->
                        <div
                            class="mt-4 p-4 bg-emerald-50/50 dark:bg-emerald-900/10 rounded-xl border border-emerald-100 dark:border-emerald-800/30"
                        >
                            <h3
                                class="font-bold text-emerald-700 dark:text-emerald-400 mb-4 uppercase text-xs"
                            >
                                Justificatifs des diplômes
                                <span
                                    class="normal-case font-normal text-[10px] text-emerald-600 dark:text-emerald-500"
                                >
                                    (PDF max 1Mo, Diplôme + Équivalence pour les
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div
                                    ...
                                    :key="dip"
                                    class="flex flex-col gap-1 p-2 bg-white dark:bg-surface-800 rounded-lg border shadow-sm dark:border-surface-600"
                                >
                                    <div
                                        class="flex justify-between items-center px-1"
                                    >
                                        <label
                                            class="text-[10px] uppercase font-bold text-gray-800 dark:text-white"
                                        >
                                            {{
                                                dip === "TS"
                                                    ? "TS (Tech. Santé)"
                                        ...
                                        <!-- ⭐ AFFICHAGE : utiliser dip (majuscule = nom colonne BD) -->
                                        <a
                                            ...
                                            "
                                            target="_blank"
                                            class="text-emerald-500 dark:text-emerald-400 text-[9px] font-bold"
                                        >
                                            <i
                                                :class="
                                                    !isOwner
                                    ...
                                    <!-- ⭐ UPLOAD : utiliser dip.toLowerCase() (nom champ contrôleur) -->
                                    <input
                                        v-if="isOwner && !hasActiveCandidature"
                                        type="file"
                                        @change="
                                        ...
                                        :disabled="form.processing"
                                    />
                                    <!-- ⭐ AFFICHAGE "Non fourni" : utiliser dip (majuscule) -->
                                    <div
                                        v-else-if="!props.user.profil?.[dip]"
                                        class="text-[9px] text-400 dark:text-surface-400 italic px-1"
                                    >
                                        Non fourni
                                    </div>
                                </div>
                ...
                <div class="mt-8 pt-6 border-t">
                    <h2
                        class="text-xl font-semibold border-b border-emerald-500/20 pb-2 text-emerald-600 flex items-center gap-2 mb-6"
                    >
                        <i class="pi pi-users"></i> Informations parentales
                    </h2>
                    <div class="mb-4">
...
.custom-file-input {
    @apply w-full text-xs text-surface-500 file:mr-3 file:py-2 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 dark:file:bg-emerald-900/30 dark:file:text-emerald-400 cursor-pointer;
}

.custom-file-input-small {
    @apply w-full text-[10px] text-surface-500 file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:text-[10px] file:font-bold file:bg-surface-100 file:text-surface-700 hover:file:bg-emerald-100 hover:file:text-emerald-700 dark:file:bg-surface-700 dark:file:text-surface-300 cursor-pointer;
}
