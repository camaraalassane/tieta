...
const primaryColor = computed(
    () => getCSSVariable("--color-primary") || "#10b981",
);
const primaryLight = computed(
    () => getCSSVariable("--color-primary-light") || "#d1fae5",
);
const primaryDark = computed(
    () => getCSSVariable("--color-primary-dark") || "#059669",
);
...
const formattedChartData = computed(() => {
    const defaultData = {
        ...
        datasets: (props.chart_data?.datasets || []).map((dataset, index) => ({
            ...dataset,
            borderColor: index === 0 ? primaryColor.value : primaryDark.value,
            backgroundColor: (context) => {
                ...
                const color =
                    index === 0 ? primaryColor.value : primaryDark.value;
...
// ⭐ Fonction utilitaire : Hex vers RGBA
...
const currentChartData = computed(() => {
    ...
    if (serviceData && serviceData.chart_data) {
        return {
            ...serviceData.chart_data,
            datasets: serviceData.chart_data.datasets.map((dataset, index) => ({
                ...dataset,
                borderColor:
                    index === 0 ? primaryColor.value : primaryDark.value,
                backgroundColor: (context) => {
...
// ⭐ Données formatées pour le graphique des candidatures (camembert)
        ...
        datasets: [
            {
                ...
                backgroundColor: [
                    ...
                    primaryColor.value, // ⭐ Couleur dynamique - Admis
                    "#ef4444", // Rouge - Rejetés
                ],
                data: props.candidatures_chart?.datasets?.[0]?.data || [
                ...
                borderColor: [
                    "#f59e0b",
                    primaryDark.value, // ⭐ Couleur dynamique
                    "#ef4444",
                ],
                borderWidth: 2,
                hoverBorderWidth: 3,
...
// ⭐ Configuration du graphique principal - DYNAMIQUE
const chartOptions = computed(() => ({
    ...
    scales: {
        x: {
            ...
            grid: {
                display: true,
                color: hexToRgba(primaryColor.value, 0.1),
                drawBorder: false,
            },
        },
        y: {
            ...
            grid: {
                color: hexToRgba(primaryColor.value, 0.15),
                drawBorder: false,
            },
        },
    },
...
// ⭐ Configuration du graphique en camembert - DYNAMIQUE
...
// ⭐ Watcher pour forcer la mise à jour du chart lors du changement de thème
...
/* ⭐ Service card selected - DYNAMIQUE */
...
/* ⭐ CHART COULEURS DYNAMIQUES - PrimeVue Chart */
:deep(.p-chart) {
    --chart-color-1: var(--color-primary);
    --chart-color-2: var(--color-primary-dark);
    --chart-color-3: var(--color-primary-light);
    --chart-color-4: #f59e0b;
    --chart-color-5: #ef4444;
}

/* Canvas chart - Forçage couleurs via CSS variables */
:deep(.p-chart canvas) {
    /* La couleur sera définie via JavaScript dans le composant */
}

/* Légende chart dynamique */
:deep(.p-chart .p-chart-legend) {
    color: var(--color-primary) !important;
}

/* BORDURE ACTIVE DYNAMIQUE - Filter badge */
.filter-badge {
    background: var(--color-primary-light) !important;
    color: var(--color-primary-dark) !important;
}
