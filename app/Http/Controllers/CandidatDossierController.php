...
class CandidatDossierController extends Controller
{
    ...
    public function show($id)
    {
        ...
        $diplomesList = [
            ...
            'DUT' => 'DUT',
            'Licence' => 'Licence',
            'Maitrise' => 'Maîtrise',  // ⭐ Nom affiché différent du nom de colonne
            'Master' => 'Master',
            'Doctorat' => 'Doctorat',
            'TS' => 'TS (Tech. Santé)',
            'TSS' => 'TSS (Tech. Sup. Santé)',
            'DES' => 'DES (Dipl. Études Spé.)',
        ];

        $diplomes = [];
        foreach ($diplomesList as $colonne => $nomAffiche) {
            $diplomes[] = [
                'nom' => $nomAffiche,     // ⭐ "Maîtrise" au lieu de "Maitrise"
                'valeur' => !empty($profil->$colonne) ? 'oui' : 'non'  // Vérifie la colonne BD
            ];
        }
