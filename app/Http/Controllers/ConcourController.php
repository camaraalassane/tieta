...
class ConcourController extends Controller implements HasMiddleware
{
    ...
    public function index(ConcourIndexRequest $request) 
    {
        ...
        // 2. On récupère les concours dont la date limite est dépassée (<= hier)
        // PUIS on ne met à jour que ceux qui sont encore "Actif"
        $hier = now()->subDay()->toDateString(); // 2026-05-10
    ...
    public function destroy(Concour $concour)
    {
        ...
        if ($currentUser->hasRole('superadmin')) {
            ...
            TracabiliteHelper::log(
                ...
                $concourData,
                null,
                $concourData['service_id'] ?? null,
                $concourData['service_nom'] ?? null
            );
